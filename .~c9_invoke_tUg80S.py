...
def index():
    if request.method == "POST":
        ...
        rows = db.execute("SELECT event FROM events WHERE ?", event)
        if len(rows) != 0:
            return
...
def record():
    ...
    if request.method == "POST":
        if request.form.get('date') and request.form.get('event') and request.form.get('expense') and request.form.get('qty') and request.form.get('cost'):
            ...
            amount = qty * cost
            pid = db.execute('INSERT INTO expenses (event, date, name, qty, cost, amount) VALUES (?, ?, ?, ?, ?, ?)',
                        event, date, expense, qty, cost, amount)
            expenses = {
                "no": f"{pid}",
                "name": f"{expense}",
                "qty": f"{qty}",
            ...
            print(expenses)
            return render_template("record.html",expenses=expenses)
        else:
